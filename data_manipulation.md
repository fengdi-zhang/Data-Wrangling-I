...
Use `?select_helpers` to see helper functions of `select`, like
...
select rows(observation), often filter using comparison operators(`>`,
`>=`, `<`, `<=`, `==`, and `!=`). `%in%` detects if values appear in a
set. `is.na()` finds missing values `&` and, `|` or, `!` negate

Some ways you might filter the litters data are: \* `gd_of_birth == 20`
\* `pups_survive != 4` \* `group %in% c("Con7", "Con8")` select rows
where group(col name) is Con7 or Con8 \*
...
To omit missing observation, use `drop_na` from `tidyr` package instead
of using `filter`. \* `drop_na(litters_data)` remove any row with any
missing value \* `drop_na(litters_data, wt_increase)` remove any row for
which `wt_increase` is missing.
